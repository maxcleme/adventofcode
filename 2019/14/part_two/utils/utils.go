...
func Fuel(recipes []Recipe, ore int) (int, error) {
	left := map[string]int{}
	i := 0
	a := ore

	for {
		// reach minimum of amount that can be created
		if a == 0 {
			break
		}

		// clone leftover before in order to be able to revert the operation
		cl := clone(left)

		// try to create 'a' amount of fuel
		c, err := minOre(recipes, a, FuelComponent, left)
		if err != nil {
			return 0, err
		}

		if c <= ore {
			// check if amount of ore is enough
			i += a
			ore -= c
		} else {
			// otherwise revert the operation and reduce the wanted amount
			a = a / 10
			left = cl
		}
	}
