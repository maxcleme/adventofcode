...
func TestMinimalDelay(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := MinimalDelay(tt.args.stringWire1, tt.args.stringWire2)
			if (err != nil) != tt.wantErr {
				t.Errorf("MinimalDelay() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("MinimalDelay() got = %v, want %v", got, tt.want)
			}
		})
	}
}
