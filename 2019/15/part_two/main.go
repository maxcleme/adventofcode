...
import (
	...
	"github.com/maxcleme/adventofcode/2019/15/part_two/utils"
	"github.com/sirupsen/logrus"
)

type TileType string

func (t TileType) Oxygeable() bool {
	switch t {
	case Droid:
		return true
	case Traveled:
...
func main() {
	...
	go func(state *EngineState) {
		for o := range cw {
			...
			case 1, 2:
				...
				if v == 1 {
					tiles[state.y][state.x] = Traveled
				} else {
					tiles[state.y][state.x] = Oxygen
	...
	minutes := 0
	oxygenPositions := [][2]int{{ox, oy}}
	for oxygenPositions != nil {
		var next [][2]int
		for _, op := range oxygenPositions {
			tiles[op[1]][op[0]] = Oxygen
			adj := AdjacentTiles(op)
			for _, t := range adj {
				if tiles[t[1]][t[0]].Oxygeable() {
					next = append(next, t)
				}
			}
		}
		oxygenPositions = next
		minutes++
		time.Sleep(time.Millisecond * 33)
		display(state.x, state.y, ox, oy, tiles)
		logrus.WithField("minutes", minutes).Info()
	}
}
...
func display(dx, dy, ox, oy int, tiles [][]TileType) {
	// clear term
...
func AdjacentTiles(position [2]int) [][2]int {
	tiles := make([][2]int, 0)
	for _, d := range AllDirections() {
		x, y := d.Apply(position[0], position[1])
